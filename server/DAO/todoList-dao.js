...
const todoListFolderPath = path.join(__dirname, "storage", "todoList");

// Method to read a todo from a file
function get(todoId) {
  try {
    const filePath = path.join(todoListFolderPath, `${todoId}.json`);
...
function create(todo) {
  try {
    ...
    const filePath = path.join(todoListFolderPath, `${todo.id}.json`);
